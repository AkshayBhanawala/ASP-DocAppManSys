...
namespace DoctorsAppointmentManager.Pages.User_Receptionist
{
	public partial class Appointments : Page
	{
		...
		private void fill_DDL_Doc_Filter()
		{
			Classes.DatabaseHandler DBH = new Classes.DatabaseHandler();
			...
			DataTable DT = DBH.Get_Filled_DT(Query);
			ListItem li_doc = new ListItem("All", "-1");
			DDL_Doc_Filter.Items.Add(li_doc);
			foreach (DataRow DR in DT.Rows)
			{
				li_doc = new ListItem(DR[0].ToString(), DR[1].ToString());
				DDL_Doc_Filter.Items.Add(li_doc);
			}
			DDL_Doc_Filter.SelectedIndex = 0;
		}
		...
		{
			Classes.DatabaseHandler DBH = new Classes.DatabaseHandler();
			string query = "SELECT ((U.FIRSTNAME)+' '+(U.LASTNAME)) AS DOCNAME, U.SPECIALITY AS DOCSPEC, U.GENDER AS DOCGEN, PA.CODE AS APPID, PR.PATIENTID AS PID, (DATENAME(DAY,PA.APPOINTMENTDATE)+'-'+CONVERT(VARCHAR,DATEPART(MONTH,PA.APPOINTMENTDATE))+'-'+DATENAME(YEAR,PA.APPOINTMENTDATE)) AS APPDATE, (DATENAME(HOUR,PA.APPOINTMENTTIME)+':'+RIGHT('0' + DATENAME(MINUTE,PA.APPOINTMENTTIME),2)) AS APPTIME, ((PR.FIRSTNAME)+' '+(PR.MIDDLENAME)+' '+(PR.LASTNAME)) AS PATIENTNAME, PR.GENDER AS GENDER, (DATEDIFF(YEAR,PR.BIRTHDATE,GETDATE())) AS AGE FROM PATIENTREGISTRATION AS PR, PATIENTSAPPOINTMENT AS PA, USERMASTER U WHERE U.CODE=PA.DOCTORID AND PR.CODE=PA.PATIENTREGISTRATIONID AND PA.PATIENTSTATUS='" + CaseStatus.Appointed + "'";
			if (DocID != -1)
			{
				query += " AND PA.DOCTORID=" + DocID;
			}
			query += " ORDER BY PA.APPOINTMENTDATE, PA.APPOINTMENTTIME";
			DataTable DT = DBH.Get_Filled_DT(query);
